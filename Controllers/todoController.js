...
export const addTodo = async (req, res) => {
    ...
    try {
        await todoSchema.create({todoMessage, creatorId: req.user._id })
...
export const showAllTodos = async (req, res) => {
    ...
    const countNumbers = await todoSchema.countDocuments()
    const allTodos = await todoSchema.find().limit(limitOfRecords).skip(skip).sort({ createdAt: -1 }).select('-__v')
...
export const showTodoByEmail = async (req, res) => {
    console.log(req.params, "===>>> req.params")
    const { email } = req.params
    const { complete } = req.query
    const todo = await todoSchema.find({ complete: complete })
...
export const deleteTodo = async (req, res) => {
    try {
        ...
        const isTodoExist = await todoSchema.findById(id)
